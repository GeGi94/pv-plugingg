...
from pv_plugingg.schema_packages.lab1_package import (
    lab1_Batch,
    lab1_Cleaning,
    lab1_Evaporation,
    lab1_Inkjet_Printing,
    lab1_LaserScribing,
    lab1_Process,
    lab1_Sample,
    lab1_SlotDieCoating,
...
class Rawlab1Experiment(EntryData):
...
class lab1ExperimentParser(MatchingParser):
    def is_mainfile(
        self,
        filename: str,
        mime: str,
    ...
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        archives = [map_batch(sample_ids, batch_id, upload_id, lab1_Batch)]
        substrates = []
        substrates_col = [
            'Sample dimension',
            'Sample area [cm^2]',
        ...
        for i, sub in df['Experiment Info'][substrates_col].drop_duplicates().iterrows():
            if pd.isna(sub).all():
                continue
            substrates.append((f'{i}_substrate', sub, map_substrate(sub, lab1_Substrate)))
        ...
        for i, row in df['Experiment Info'].iterrows():
            ...
            archives.append(map_basic_sample(row, substrate_name, upload_id, lab1_Sample))
        ...
        for i, col in enumerate(df.columns.get_level_values(0).unique()):
            ...
            for j, row in df_dropped.iterrows():
                ...
                if 'Cleaning' in col:
                    archives.append(map_cleaning(i, j, lab_ids, row, upload_id, lab1_Cleaning))

                if 'Laser Scribing' in col:
                    archives.append(map_laser_scribing(i, j, lab_ids, row, upload_id, lab1_LaserScribing))

                if 'Generic Process' in col:  # move up
                    generic_process = map_generic(i, j, lab_ids, row, upload_id, lab1_Process)
                ...
                if 'Evaporation' in col:
                    ...
                    if 'Co-Evaporation' in col:
                        coevap = True
                    archives.append(
                        map_evaporation(i, j, lab_ids, row, upload_id, lab1_Evaporation, coevap)
                    )

                if 'Spin Coating' in col:
                    archives.append(map_spin_coating(i, j, lab_ids, row, upload_id, lab1_SpinCoating))

                if 'Slot Die Coating' in col:
                    archives.append(map_sdc(i, j, lab_ids, row, upload_id, lab1_SlotDieCoating))

                if 'Sputtering' in col:
                    archives.append(map_sputtering(i, j, lab_ids, row, upload_id, lab1_Sputtering))

                if 'Inkjet Printing' in col:
                    archives.append(
                        map_inkjet_printing(i, j, lab_ids, row, upload_id, lab1_Inkjet_Printing)
                    )

                if 'ALD' in col:
                    archives.append(
                        map_atomic_layer_deposition(
                            i, j, lab_ids, row, upload_id, lab1_AtomicLayerDeposition)
                    )
        ...
        archive.data = Rawlab1Experiment(processed_archive=refs)
