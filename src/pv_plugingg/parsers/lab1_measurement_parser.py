...
class RawFilelab1(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class lab1Parser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = lab1_Measurement()

        if measurment_type == 'jv':
            entry = lab1_JVmeasurement()
        if measurment_type == 'eqe':
            entry = lab1_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = lab1_SimpleMPPTracking()
        ...
        archive.data = RawFilelab1(processed_archive=get_reference(archive.metadata.upload_id, eid))
